...
######### Combining results from multiple models 2

pred_rf = predict(model_rf2, test)
pred_xgb = predict(model_xgb_ultra_final, test)
pred_glmnet = predict(model_glm, test)
pred_boosted_glmnet = predict(model_boost_glm, test)

all_preds = data.frame(RF = pred_rf, XGB = pred_xgb, GLM = pred_glmnet, GLM_BOOST = pred_boosted_glmnet, Target = test$cc_cons)
...
saveRDS(model_stack, "model_stack_xgb4.rds")
...
saveRDS(model_stack2, "model_stack_glm4.rds")

# Stacking using GLM

model_stack2 = train(log(Target + 1) ~., data = train2, method = "glmboost", trControl = trainctrl, metric = "RMSE")
...
saveRDS(model_stack2, "model_stack_glm_boost2.rds")
